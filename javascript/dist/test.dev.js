...
function binarySearchTreeSort2nd(ary) {
  var root = null;
  root = ary.reduce(function (bst, item) {
    return insertIntoBST(bst, item);
  }, root);
  var result = [];
...
var invertTree = function (root) {
  ...
  while (true) {
    ...
    var dummy = [].concat(nextRow);
    var _iteratorNormalCompletion = true;
    var _didIteratorError = false;
    var _iteratorError = undefined;

    try {
      for (var _iterator = curRow[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var _node = _step.value;
        _node.left = dummy.shift();
        _node.right = dummy.shift();
      }
    } catch (err) {
